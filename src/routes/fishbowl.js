...
function setupFishbowlRoutes(logger) {
  // Unsafe SQL query (deprecated)
  router.post('/unsafe/deprecated/data-query', async (req, res) => {
    const { serverUrl, token, sql } = req.body;

    logger.api('UNSAFE SQL QUERY', {
      serverUrl,
      sqlPreview: sql.substring(0, 150) + '...',
      url: `${serverUrl}/api/unsafe/deprecated/data-query`
    });

    try {
      const url = new URL(`${serverUrl}/api/unsafe/deprecated/data-query`);
        ...
        const apiReq = httpModule.request(options, (apiRes) => {
          ...
          apiRes.on('end', () => {
            try {
              ...
              const resultType = Array.isArray(parsed) ? 'Array' : typeof parsed;
              logger.api(`UNSAFE SQL QUERY RESPONSE - Status: ${apiRes.statusCode}, Type: ${resultType}`);

              if (apiRes.statusCode !== 200) {
                logger.error('UNSAFE SQL QUERY ERROR RESPONSE', { response: parsed });
              }
              resolve(parsed);
            } catch (e) {
              logger.error('UNSAFE SQL QUERY - Invalid JSON response', {
                error: e.message,
      ...
      logger.error('UNSAFE SQL QUERY ERROR', { error: error.message, stack: error.stack });
